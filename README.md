...
## instruction on how to run our program
First, boot up xv6 by typing in to terminal:
$make clean
$make
$make qemu
Then, $strace on and then type in any valid command to trace, or $strace run "command" which activates tracing for just that command.
type in $strace off to turn off tracing mode. Type in $strace dump to see the printout of the N number of latest events. (currently N = 16).
You can find the variable N defined in "proc.h" file. Also, strace options such as "-e", "-s", and "-f" are alsoo available.
